...
- 公開済み: **v1.57.1**（v1.56.0 遅れ係数を提出物チェックだけに／v1.57.0 取り消し点を種類ごとの枠に／v1.57.1 診断に遅れ係数の影響の表示）。push 済み（2026-09-20・先生はバックアップと変更前の1学期成績CSVを保存済み）。`git log origin/main..HEAD` で確認する。**iPad 確認済みは v1.55.0 まで**（v1.55.1 以降は未確認）。
