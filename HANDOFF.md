# classroom-app 引き継ぎメモ（2026-09-20 時点・公開済みは v1.55.1／手元は v1.57.0）
...
- 公開済み: **v1.55.1**。手元の `main` は **v1.57.0（v1.56.0: 遅れ係数を提出物チェックだけに／v1.57.0: 取り消し点を種類ごとの枠に分ける。どちらも commit 済み・push は先生の承認待ち）**。`git log origin/main..HEAD` で確認する。**iPad 確認済みは v1.55.0 まで**（v1.55.1 以降は未確認）。
...
| 追加 | 取り消し点を種類ごとの別の枠に分ける（互いに上書きしない）。入力形式の取り消しは編集画面へ | v1.57.0（未push） | 26 |
