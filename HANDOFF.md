...
- 先生は 4b（v1.53.0）以降を入れる前にバックアップ済み。**iPad での確認結果はまだ受け取っていない**。
  確認してもらうこと（UI_WORKLOG の手順どおり）:
  1. バージョン表示が v1.53.1 になる。開き直した直後に「新体力テストの記録を…新しい方式に更新しました」の通知が出る。
  2. 設定の「🔎 新体力テストの連携状態」で、記録の管理方式が「児童ごと（新方式）」、名簿は「一致しています」、
     「ほかの全データ ○件は変化なし」と出る。
  3. 「新体力テスト」タブの27名の記録が、更新前のスクリーンショットと同じ児童に付いている。
  4. 名簿を変更せず保存すると「変更はありません」。名簿の編集直後の通知が出ている間でも保存ボタンが押せる。
