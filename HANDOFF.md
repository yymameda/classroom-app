# classroom-app 引き継ぎメモ（2026-09-20 時点・公開済みは v1.58.0／未 push なし）
...
- 全体は `node run-all.js`（64ファイル・2182件・約13分。件数は実行結果と README の表で確認する）。新しいテストの出力形式が読み取れない場合は `run-all.js` の `parseCounts` に形式を追加する。Chrome は `/Applications/Google Chrome.app`。
...
1. **v1.58.0（4d-2）の未確認2項目**: 復元時の整合性確認カード・バックアップの書き出しを、先生が次にバックアップを取る／復元する機会に iPad で確認してもらう（push 済み。版表示と監査診断は確認済み）。
2. **M2〜M5**（提出率の定義・面談テキストの学期・レーダーPDFの教科カード・同僚用テンプレート）: **成績や帳票の数字が変わるので、方針を先生に確認してから**。M2（提出率を3系統から1つに）が先。
3. **4e**（`pf.html` を案内ページに。4d-2 の次の最優先）。その後、`pf-residual`・暫定ガードの整理と、DATA_FLOW_AUDIT.md の 7.5 の実施順の更新。
