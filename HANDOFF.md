# classroom-app 引き継ぎメモ（2026-09-20 時点・公開済みは v1.55.1／手元は v1.56.0）
...
- 公開済み: **v1.55.1**。手元の `main` は **v1.56.0（遅れ係数0.8倍を提出物チェックだけに。commit 済み・push は先生の承認待ち）**。`git log origin/main..HEAD` で確認する。**iPad 確認済みは v1.55.0 まで**（v1.55.1 以降は未確認）。
...
| 追加 | 遅れ係数0.8倍を「提出物チェックの提出記録」だけに（児童の記録には掛けない）(M8) | v1.56.0（未push） | 25 |
