# classroom-app 引き継ぎメモ（2026-09-20 時点・公開済みは v1.54.2）
...
- 公開済み: **v1.54.2**（診断の拡張。push 済みなら `git log origin/main..HEAD` は空）。iPad 確認済みは v1.53.1 まで。
...
| 追加 | H7 の診断の拡張（5段階で数値の記録・成績への影響・直し方の案内） | v1.54.2 | 23 |
...
- **判断待ち（23章）**: 入力形式の固定のため、次の2状態は画面操作で直せない。(c) 5段階（判定）なのに数値の記録がある課題（成績に影響。主体性は成績に入らない等）、(d) 得点方式なのに文字（ABC）の記録がある課題（満点を入れると診断から消えるが記録は食い違ったまま）。
  案: 記録の型と入力形式が食い違う課題に限りタブの固定を外す／診断に「入力形式を直す」ボタン（確認つき・取り消し可）／(d) を検出条件に追加。先生の判断後に実装する。
...
1. 先生の iPad で診断（データ出力（左の列の📤）の画面の一番下の「🔍 監査診断」（押して開く）>「診断を実行」。設定画面ではない）の表示を見てもらい、「入力形式と満点が矛盾する課題」の件数・内容を聞く。直せない状態（23章の (c)(d)）が実際にあれば、対応を決めて実装する。
