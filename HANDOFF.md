# classroom-app 引き継ぎメモ（2026-09-20 時点・公開済みは v1.57.1／commit 済み・未 push は v1.58.0）
...
- **未 push（commit まで・先生の承認待ち）: v1.58.0**（H4 4d-2: バックアップの版の印 version 11・版の判別・復元後の整合性検査と設定の確認カード。DATA_FLOW_AUDIT.md 27章）。iPad 未確認。
...
### 残り（4e のみ。4c は公開済み・4d-2 は v1.58.0 で commit 済み・未 push）

- 4c の実装内容は DATA_FLOW_AUDIT.md 21章。先生の端末は名簿が一致して自動移行済みのため、4c の画面は今の端末には出ない。
- （実装済み・未 push）**4d-2**（16.6・27章）: 書き出しは `version: 11`・`studentIdSchema: 1`・`pfSchema`（端末の実際の方式）。復元は版の数字でなく中身で新旧を判別し、案内だけ出す（止めない）。復元後に `spaIntegrityCheck`（読み取り専用）で児童IDの欠落・重複、pf の新旧混在、退避の二重・破損を検査し、トーストと設定の「データの整合性の確認」カードで知らせる。自動では直さない。
...
1. **v1.58.0（4d-2）の push 判断**: 先生の承認後に push。実機の挙動が変わる段階なので「バックアップ → 更新 → 確認」の順で案内（確認: 設定に「🩺 データの整合性の確認」が出て「問題は見つかりませんでした」になること。バックアップの書き出しは従来どおり）。
