# classroom-app 引き継ぎメモ（2026-09-20 時点・公開済みは v1.54.3／手元は v1.55.0）
...
- 公開済み: **v1.54.3**（診断の誤判定の修正）。手元の `main` は **v1.55.0（記録に合わせて入力形式を戻す機能。commit 済み・push は先生の承認待ち）**。`git log origin/main..HEAD` で確認する。iPad 確認済みは v1.53.1 まで。
...
| 追加 | 記録と入力形式が食い違う課題を、記録に合う形式へ戻す（取り消しつき） | v1.55.0（未push） | 24 |
...
- （実装済み・push待ち）入力形式の固定で直せなかった状態は、v1.55.0 で「記録に合わせて…入力に戻す」ボタンにより直せる（24章）。直せないのは記録が数値と文字の混在・有効でない文字の課題だけ（診断に「画面の操作では直せない」と出る）。
