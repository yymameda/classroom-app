# classroom-app 引き継ぎメモ（2026-09-20 時点・公開済みは v1.57.1）
...
1. **4d-2**（バックアップに版の印・復元後の整合性検査。§2 の「残り」）: 設計を確認して実装（commit まで → 報告 → 承認 → push）。
2. **M2〜M5**（提出率の定義・面談テキストの学期・レーダーPDFの教科カード・同僚用テンプレート）: **成績や帳票の数字が変わるので、方針を先生に確認してから**。M2（提出率を3系統から1つに）が先。
3. **4e**（`pf.html` を案内ページに）。その後、`pf-residual`・暫定ガードの整理と、DATA_FLOW_AUDIT.md の 7.5 の実施順の更新。
