// 診断の情報表示「まとめテスト … 全問が単一観点(観点不均衡の可能性)」の見直し(v1.55.1)。
...
(async () => {
    ...
    try {
        ...
        const spy = await page.evaluate(async () => {
            ...
            return { writes: n, info: r2.anomalies.info, perTest: r2.matomeBalance.perTest.map(p => [p.testId, p.name, p.total]) };
        });
        const info = spy.info.filter(i => /まとめテスト testId=/.test(i) && /単一観点|観点ですが/.test(i));
        ...
        check('課題名に観点の語が無く全問が単一観点: これまでどおり表示する(課題名つき)', has(7).length === 1 && /全問が単一観点\(観点不均衡の可能性\)/.test(has(7)[0]) && /「まテ_単元のまとめ」/.test(has(7)[0]), JSON.stringify(has(7)));
