...
(async () => {
    ...
    try {
        ...
        // ================================================================
        // 7. 「作文」に関わるコードが変更されていないことの回帰確認
        //    (作文テストを作成し、従来どおり数値入力・満点入力必須のままであること)
        // ================================================================
        await fillTestForm({ subject: '国語', testType: '作文', name: 'ISABC8_作文回帰', category: '思考・判断・表現', date: '2026-06-01' });
        await page.evaluate(() => { window.recAddTest(); }); // maxScore未入力のまま保存を試みる
        await new Promise(r => setTimeout(r, 150));
        const compositionRejected = await findTestByName('作文_ISABC8_作文回帰');
        check('作文回帰: 満点未入力では保存されない(従来どおり)', compositionRejected === null, JSON.stringify(compositionRejected));
        await page.evaluate(() => { document.getElementById('recTestMaxScore').value = '5'; });
        await page.evaluate(() => { window.recAddTest(); });
        await new Promise(r => setTimeout(r, 150));
        const compositionCreated = await findTestByName('作文_ISABC8_作文回帰');
        check('作文回帰: 満点5を入力すれば通常どおり数値入力形式で保存される(maxScore=5)', compositionCreated && compositionCreated.maxScore === 5, JSON.stringify(compositionCreated));
        await page.evaluate((id) => { window.recSelectTestGoto(id); }, compositionCreated.id);
        await new Promise(r => setTimeout(r, 150));
        const compositionUi = await page.evaluate(() => ({
            hasScInput: !!document.getElementById('rec-sc-0'),
            abcBtnCount: document.querySelectorAll('#rec-row-0 .rec-abc-btn').length
        }));
        check('作文回帰: 採点画面は従来どおり数値入力欄(ABCボタンではない)', compositionUi.hasScInput === true && compositionUi.abcBtnCount === 0, JSON.stringify(compositionUi));
